...
//        if 0 <= nCode && (keydownobj <> null || keypressobj <> null || keyupobj <> null) then
        if nCode = 0 then
            ...
            // KeyDownイベントを発行
//            if keydownobj <> null && (msg = WindowsAPI.WM_KEYDOWN || msg = WindowsAPI.WM_SYSKEYDOWN) then
            if msg = WindowsAPI.WM_KEYDOWN || msg = WindowsAPI.WM_SYSKEYDOWN then
                ...
                Debug.WriteLine <| sprintf "KeyboardHook[down]: %A" e
                _keyDownEvent.Trigger e
            // KeyPressイベントを発行
//            if keypressobj <> null && msg = WindowsAPI.WM_KEYDOWN then
            ...
            // KeyUpイベントを発行
//            if keyupobj <> null && msg = WindowsAPI.WM_KEYUP then
...
//#endregion
    ...
    let GetForegroundProcess() =
        ...
        |> (fun hwnd -> WindowsAPI.GetWindowThreadProcessId(hwnd, IntPtr.Zero))
